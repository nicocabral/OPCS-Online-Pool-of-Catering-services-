...
<table id="datatable-fixed-col" width="100%;" class="table table-striped table-bordered" style="box-shadow: 3px 2px 2px #000;cursor:pointer;" >
                               ...
                               <tbody id="order_res" class="wow fadeInDown">
								...
								$query = 'SELECT * FROM tbl_orders WHERE catererid = "'.$_SESSION['id'].'" AND order_status = "Rejected"';
								...
								<tr class="wow fadeInDown" style="color:#000;">
									...
									<center>
									<div class="btn-group">
									<button class="btn btn-danger btn-sm waves-effect waves-light"  style="background-color:#821a1a;" onclick="vieworder_rejected(<?php echo $row['orderid']?>,<?php echo $row['catererid']?>,'<?php echo $row['order_name']?>','<?php echo $row['order_price']?>','<?php echo $row['order_qty']?>','<?php echo $row['customer_name']?>','<?php echo $row['customer_contact']?>','<?php echo $row['customer_address']?>','<?php echo $row['customer_message']?>','<?php echo $row['order_date']?>','<?php echo $row['order_payment']?>','<?php echo $row['order_status']?>','<?php echo $row['order_timestamp']?>','<?php echo $row['time']?>');"><i class="fa fa-search"></i> View</button>
									</div></center>
</td>
								</tr>
...
function vieworder_rejected(o_id,cid,oname,oprice,oqty,cname,cno,cad,cmsg,od,op,os,ot,time){
