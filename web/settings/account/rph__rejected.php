...
<table id="datatable-fixed-col" width="100%;" class="table table-striped table-bordered" style="box-shadow: 3px 2px 2px #000;cursor:pointer;" >
                                <thead style="background-color:#821a1a; color:#fff;">
                                <tr>
                               ...
                               <tbody id="reservation_res" class="wow fadeInDown">
								...
								$query = 'SELECT tbl_reservation.*,tbl_ocasioncat.* FROM tbl_reservation LEFT JOIN tbl_ocasioncat ON tbl_ocasioncat.oid = tbl_reservation.ocasion_id WHERE tbl_reservation.res_status = "Rejected" AND tbl_reservation.catererid ='.$_SESSION['id'].'';
								...
								<tr class="wow fadeInDown" style="color:#000;">
									...
									<td style="width:auto;"><?php echo '&#8369;'.number_format($row['price'],2,'.',',');?></td>
									...
									<td style="width:auto;">
									<p class="label label-danger" style="font-size:13px;"><?php echo $row['res_status']?></p></td>
									<td>
									<center>
									<div class="btn-group">
									<button class="btn btn-danger btn-sm waves-effect waves-light" onclick="viewReservation(<?php echo $row['reservationid']?>,<?php echo $row['catererid']?>,'<?php echo $row['clientname']?>',<?php echo $row['month'] ?>,<?php echo $row['date']?>,'<?php echo $row['year']?>','<?php echo $row['time']?>','<?php echo $row['address']?>','<?php echo $row['email']?>','<?php echo $row['contact']?>','<?php echo $row['res_status']?>','<?php echo $row['message']?>','<?php echo $row['servicepackno']?>','<?php echo $row['food_packno']?>',<?php echo $row['price']?>,<?php echo $row['ocasion_id'];?>,'<?php echo $row['ocassion_name']?>',<?php echo $row['servicepackno']?>);" style="background-color:#821a1a;"><i class="fa fa-trash"></i></button>
									</div></center>
</td>
								</tr>
