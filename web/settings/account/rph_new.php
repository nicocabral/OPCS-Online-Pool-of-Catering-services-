...
<table id="datatable-fixed-col" width="100%;" class="table table-striped table-bordered" style="box-shadow: 3px 2px 2px #000;cursor:pointer;" >
                               ...
                               <tbody id="reservation_res" class="wow fadeInDown">
								...
								$query = 'SELECT * FROM tbl_customereservation WHERE res_mycatid ='.$_SESSION['id'].' AND res_status = "New"';
...
function confirmrph(resid){
	swal({
  title: "Confirmation",
  text: "Reservation will be moved to pending until service completed",
  type: "warning",
...
function(){
	$.ajax({
		url:'process/confirmrph_process?resid='+resid,
		type:'POST',
		cache:false,
...
function rejectrph_new(resid){
	swal({
  title: "Confirmation",
  text: "Are you sure you want to reject this reseravtion?",
  type: "warning",
  showCancelButton: true,
  confirmButtonClass: "btn-danger",
...
function(){
 $.ajax({
		url:'process/rejectRph_process?resid='+resid,
		type:'POST',
		cache:false,
		success:function(data){
			$('#viewReservationModal').modal('hide');
